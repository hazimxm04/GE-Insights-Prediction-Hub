"""
train_johor_weighted_backtest.py
==================================
BACKTEST: Trains Johor using the SAME weighted-recency
multi-transition methodology built for Selangor/Melaka,
then validates the resulting forecast against Johor's
REAL 2026 election results.

This directly tests whether the improved methodology
(weighted vote share trend + native model + loyalty blend)
outperforms, matches, or underperforms the ORIGINAL
single-transition approach that achieved 94.64% accuracy.

Training transitions: 2008->2013, 2013->2018, 2018->2022
(predicting historical elections using the new methodology)

Then forecast 2026 using 2022 as the "current" baseline,
compare against REAL 2026 results (validation_2026.csv).
"""
...
print("BACKTEST: JOHOR (weighted-recency methodology)")
print("="*70)

print("Loading Johor election data...")
url_ballots = 'https://lake.electiondata.my/results_headline/headline_ballots_state_jhr.parquet'
url_stats   = 'https://lake.electiondata.my/results_headline/headline_stats_state_jhr.parquet'
...
print(f"  2008->2013: {len(df_2013)} seats, Harapan wins={df_2013['target_non_bn_won'].sum()}")
...
df_train = pd.concat([df_2013, df_2018, df_2022], ignore_index=True)
...
df_train = merge_ethnicity_into_features(
    df_features=df_train, state='johor', year_b=2022,
...
print(f"  Class balance: Harapan={y_train.sum()}, Non-Harapan={len(y_train)-y_train.sum()}")
...
# ── BACKTEST: Forecast 2026 using ONLY pre-2026 data ───────────────

print("\nBuilding 2026 forecast (using 2022 as 'current' baseline)...")

df_forecast = build_transition(2018, 2022)  # same structural base as validated model

df_forecast, recent_year = get_historical_ph_vote_shares(
    ballots, df_forecast, seat_col='seat', recent_year_cutoff=2023
)
...
df_forecast = merge_ethnicity_into_features(
    df_features=df_forecast, state='johor', year_b=2022,
    sentiment=sentiment, economic_pressure=economic
)
...
backtest_df['backtest_prediction'] = backtest_df['backtest_probability'].apply(
    lambda x: 'non-BN' if x >= 0.5 else 'BN'
)

# ── Compare against REAL 2026 results ──────────────────────────────

print("\nComparing backtest forecast vs REAL validated 2026 results...")

actual_df = pd.read_csv(ROOT / 'backend/models/johor/validation_2026.csv')
...
out_path = ROOT / 'data/processed/johor_backtest_comparison.csv'
