"""
train_selangor_model.py
========================
v2: Trains on TWO historical transitions (2013->2018 AND
    2018->2023), doubling training data to ~112 rows, with
    stronger regularization to reduce overfitting/overconfidence
    seen in the v1 single-transition model (which produced
    unreasonably extreme probabilities like 0.95-0.98 on
    genuinely competitive seats).

Both transitions use the same PH-era political landscape
(post-2013, PR->PH formation already settled), avoiding the
noisier pre-2013 period with different coalition dynamics.
"""
...
print("TRAINING: SELANGOR (native model, v2 - more data + regularization)")
print("="*70)
...
def build_transition(year_a, year_b):
    """Build one training transition (year_a -> year_b) as features."""
...
print(f"  2018->2023: {len(df_2023)} seats, Harapan wins={df_2023['target_non_bn_won'].sum()}")

df = pd.concat([df_2018, df_2023], ignore_index=True)
print(f"\n  Combined training set: {len(df)} rows")

# ── Step 2: Sentiment + economic + ethnicity (same for both transitions) ──
...
# ── Step 3: Train models with stronger regularization ─────────────
...
rf = RandomForestClassifier(
    ...
    min_samples_split=8,   # new: require more samples to split
...
ensemble_acc = (
    ((rf_probs + xgb_probs) / 2 >= 0.5).astype(int) == y
).mean()
...
print(f"  (Less extreme range than v1 indicates reduced overconfidence)")
...
print("Trained on 2 transitions (2013->2018, 2018->2023) with")
